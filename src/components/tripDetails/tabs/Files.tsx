  ...
  return (
    <div className="px-8 w-full space-y-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage all your bookings and trip assets
          </p>
        </div>
        <Button
      ...
      <div
        ...
        } ${selectedFile ? "border-green-300 bg-green-100/20" : ""}`}
      >
        {!selectedFile ? (
          <>
            ...
            <div className="relative">
              <Button
                variant="secondary"
                className="bg-blue-100/50 text-blue-700 hover:bg-blue-100"
              >
                Select Files
              </Button>
              <input
        ...
        <TabsList className="bg-transparent gap-4">
          {CATEGORIES.map((cat) => (
            <TabsTrigger
              key={cat}
              value={cat}
              className="rounded-full px-6 border data-[state=active]:bg-slate-900 data-[state=active]:text-white transition-all cursor-pointer"
            >
              {cat}
            </TabsTrigger>
          ))}
