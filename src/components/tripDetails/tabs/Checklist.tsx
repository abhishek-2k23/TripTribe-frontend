...
const completedItems = category.items.filter(i => i.isCompleted).length;
const progress = totalItems > 0 ? (completedItems / totalItems) * 100 : 0;
                  ...
                  <div key={item._id} className="flex items-center justify-between group">
                    <div className="flex items-center gap-3">
                      <Checkbox 
                        ...
                        onCheckedChange={(val) => toggleItem( category._id, item._id, !!val)}
                      />
                    ...
                    {item.isCompleted && item.completedBy && (
                      <Avatar className="h-6 w-6 border">
                        ...
                        <AvatarFallback>{item.completedBy.name[0]}</AvatarFallback>
                      </Avatar>
                    )}
                  </div>
                ))}
      ...
      <AddChecklistModal isOpen={isModalOpen} onClose={() => setModalOpen(false)} tripId={selectedTripId} />
    </div>
  );
}